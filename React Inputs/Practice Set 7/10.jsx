...
export default function App() {
  ...
  const getProductsData = async () => {
    try {
      ...
      if (status === 200) {
        ...
        setDisplaySearchProduct(data.products);
      }
    } catch (e) {
      console.log(e);
    }
  ...
  const handleUserInput = (e) => {
    ...
    setDisplaySearchProduct(filteredProducts);
  };

  return (
    <div>
      <h1> Products </h1>
      <input placeholder="Search Products" onChange={handleUserInput}></input>
      {displaySearchProduct.length === 0 ? (
        <p>No products found</p>
      ) : (
        displaySearchProduct.map(({ name, price, quantity, rating }) => (
          <li
            key={name}
            style={{
